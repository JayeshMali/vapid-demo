class ContentController < ApplicationController
	def sendPush
		...
		# notif_data = NotificationDatum.last
		# User.where(auth_key: notif_data.auth_key).destroy_all
		# user = User.create(auth_key: notif_data.auth_key, :notif_id => notif_data.id)
		sendPayload(user)
		render body: nil
	end
	...
	def sendPayload(user)
	    ...
	    if user.notif_id.present?
	      ...
	      Webpush.payload_send(endpoint: @notification_data.endpoint,
	                           ...
	                           ttl: 24 * 60 * 60,
	                           vapid: {
	                               subject: 'mailto:jayesh.mali@metacube.com',
    ...
    def get_message(name)
    	"Hello World Sample Notification"
    end
end
